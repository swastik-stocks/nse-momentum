"""
validation/fip_validate.py — Factor-library backlog item validation

Tests FIPAgent (agents/fip_agent.py) — Da, Gurun & Warachka (2014)'s
"Frog in the Pan" continuous-vs-discrete information measure — against the
same gate-cleared signal set every other backlog item used (pattern + RS +
liquidity + risk + asymmetry + VCP, price_history_deep, 2007-2026), reusing
the real agent classes directly.

The paper's finding: momentum continuation is stronger when the prior move
was built from many small same-direction days (smooth) than from a few big
jumps (discrete), even at identical total magnitude. This harness buckets
gate-cleared signals by fip_score tercile (post-hoc, computed across all
collected signals — not a fixed a-priori scale) and tests whether the
smoothest tercile actually outperforms.

HONEST SCOPE NOTE (see also the agent's own docstring): the paper tests
monthly-rebalanced US-equity decile portfolios over 1926-2011 — a very
different sampling scheme from this codebase's event-driven NSE breakout
signals. Never validated on NSE microstructure before this run.

Usage:
    python validation/fip_validate.py
    python validation/fip_validate.py RELIANCE
    python validation/fip_validate.py --stats
    python validation/fip_validate.py --fresh
"""
...
from agents.fip_agent import compute_stock_fip_components, compute_universe_fip_index
...
_log_file = _LOG_DIR / f"fip_validate_{datetime.now().strftime('%Y-%m-%d_%H%M%S')}.log"
...
_PROGRESS_DDL = """
CREATE TABLE IF NOT EXISTS fip_validate_progress (
...
def _clear_progress():
    conn = get_connection()
    conn.execute(_PROGRESS_DDL)
    conn.execute("DELETE FROM fip_validate_progress")
    conn.commit()
    conn.close()
...
def _load_completed_tickers() -> set:
    conn = get_connection()
    rows = conn.execute("SELECT ticker FROM fip_validate_progress").fetchall()
    conn.close()
...
def _save_ticker_progress(ticker: str, res: dict) -> None:
    conn = get_connection()
    conn.execute("""
        INSERT OR REPLACE INTO fip_validate_progress
...
def _load_all_progress() -> list:
    conn = get_connection()
    rows = conn.execute(
        "SELECT ticker, signals, results_json FROM fip_validate_progress"
    ).fetchall()
    conn.close()
    return [
...
def _precompute_fip_index_by_date(dates: list, components_by_ticker: dict, tickers: list) -> dict:
    log.info("  Precomputing per-date cross-sectional fip_score...")
    result = {}
    sampled_dates = dates[::STEP]
    for idx, date in enumerate(sampled_dates):
        date_str = date.strftime("%Y-%m-%d")
        result[date_str] = compute_universe_fip_index(components_by_ticker, date, tickers)
        if idx % 20 == 0:
...
def replay_ticker(ticker: str, tier: str, nifty_df: pd.DataFrame,
                   ...
                   fip_index_by_date: DateIndexedRanks,
    ...
    for i in range(150, len(df) - FORWARD_BARS, STEP):
        ...
        fip_day = fip_index_by_date.get_day(date_str)
        fip_score = fip_day.get(ticker)  # None if missing — not zero-filled
        ...
        result = {
            ...
            "penalty_applied": penalty, "fip_score": fip_score,
...
def run_replay(tickers: list = None, fresh: bool = False) -> None:
    ...
    log.info("Precomputing rolling fip_score per ticker (vectorized, once each)...")
    components_by_ticker = {
        t: compute_stock_fip_components(df) for t, df in stock_data.items()
    }
    fip_index_raw = _precompute_fip_index_by_date(all_dates, components_by_ticker, list(stock_data.keys()))
    fip_index_by_date = DateIndexedRanks(fip_index_raw)

    log.info(f"Running FIP validation on {len(remaining)} stocks...")
    total_signals = 0

    for i, (ticker, name, sector, tier) in enumerate(remaining, 1):
        try:
            res = replay_ticker(ticker, tier, nifty_df, universe_ranks_by_date,
                                 fip_index_by_date, regime_map)
...
def analyze_and_store() -> None:
    ...
    with_fip = [o for o in all_outcomes if o.get("fip_score") is not None]

    if len(with_fip) < 30:
        log.warning(f"Only {len(with_fip)} signals have a fip_score — too few to analyze.")
        return

    report = {"generated_at": datetime.today().isoformat(),
              "total_signals": len(all_outcomes), "with_fip_score": len(with_fip)}
    rng = np.random.default_rng(RANDOM_SEED)

    pool_r = np.array([o["net_r"] for o in with_fip], dtype=float)
    report["pool"] = _bucket_stats(with_fip)

    # Post-hoc terciles — fip_score is bounded [-1, 1] but its realized
    # distribution within an already-curated (breakout-cleared) population
    # isn't known a priori, so cutoffs come from THIS run's collected
    # signals, same convention as lottery_index_validate.py.
    values = np.array([o["fip_score"] for o in with_fip])
    p33, p67 = np.percentile(values, [33, 67])
    # Top tercile = HIGHEST fip_score = smoothest trend = paper's thesis
    top    = [o for o in with_fip if o["fip_score"] >= p67]
    mid    = [o for o in with_fip if p33 < o["fip_score"] < p67]
    bottom = [o for o in with_fip if o["fip_score"] <= p33]
    ...
    report["top_tercile_fip"] = top_stats       # smoothest trend
    report["mid_tercile_fip"] = mid_stats
    report["bottom_tercile_fip"] = bottom_stats  # choppiest trend
    ...
    conn = get_connection()
    conn.execute("""
        CREATE TABLE IF NOT EXISTS fip_validate_results (
    ...
    """)
    conn.execute(
        "INSERT INTO fip_validate_results (report_json, generated_at) VALUES (?,?)",
        (json.dumps(report), report["generated_at"])
    )
    conn.commit()
    conn.close()
    log.info("Analysis stored in fip_validate_results.")


def print_stats() -> None:
    conn = get_connection()
    row = conn.execute(
        "SELECT report_json FROM fip_validate_results ORDER BY id DESC LIMIT 1"
    ).fetchone()
    conn.close()
    if not row:
    ...
    print("\n" + "=" * 95)
    print("  FACTOR-LIBRARY BACKLOG — Frog-in-the-Pan / Information Discreteness (Da, Gurun & Warachka 2014)")
    print("=" * 95)
    print(f"  Total gate-cleared signals: {report.get('total_signals', 0)} "
          f"({report.get('with_fip_score', 0)} with a computable fip_score)")
