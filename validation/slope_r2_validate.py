"""
validation/slope_r2_validate.py — Factor-library backlog item validation

Tests SlopeR2Agent (agents/slope_r2_agent.py) — Clenow's exponential
regression slope x R^2 momentum formula ("Stocks on the Move") — against
the same gate-cleared signal set every other backlog item used (pattern +
RS + liquidity + risk + asymmetry + VCP, price_history_deep, 2007-2026),
reusing the real agent classes directly.

Clenow's thesis: a smooth, well-fit uptrend (high R^2) continues more
reliably than an equal-magnitude but choppy one (low R^2). This harness
buckets gate-cleared signals by slope_r2_score tercile (post-hoc, computed
across all collected signals) and tests whether the top tercile actually
outperforms.

HONEST SCOPE NOTE: Clenow's own validated use case is a monthly-rebalanced
S&P 500 trend-following system, not event-driven NSE breakout signals —
never tested on this codebase's population before this run.

Usage:
    python validation/slope_r2_validate.py
    python validation/slope_r2_validate.py RELIANCE
    python validation/slope_r2_validate.py --stats
    python validation/slope_r2_validate.py --fresh
"""
...
from agents.slope_r2_agent import compute_stock_slope_r2_components, compute_universe_slope_r2_index
...
_log_file = _LOG_DIR / f"slope_r2_validate_{datetime.now().strftime('%Y-%m-%d_%H%M%S')}.log"
...
_PROGRESS_DDL = """
CREATE TABLE IF NOT EXISTS slope_r2_validate_progress (
...
def _clear_progress():
    conn = get_connection()
    conn.execute(_PROGRESS_DDL)
    conn.execute("DELETE FROM slope_r2_validate_progress")
    conn.commit()
    conn.close()
...
def _load_completed_tickers() -> set:
    conn = get_connection()
    rows = conn.execute("SELECT ticker FROM slope_r2_validate_progress").fetchall()
    conn.close()
...
def _save_ticker_progress(ticker: str, res: dict) -> None:
    conn = get_connection()
    conn.execute("""
        INSERT OR REPLACE INTO slope_r2_validate_progress
...
def _load_all_progress() -> list:
    conn = get_connection()
    rows = conn.execute(
        "SELECT ticker, signals, results_json FROM slope_r2_validate_progress"
    ).fetchall()
    conn.close()
    return [
...
def _precompute_slope_r2_index_by_date(dates: list, components_by_ticker: dict, tickers: list) -> dict:
    log.info("  Precomputing per-date cross-sectional slope*R^2 score...")
    result = {}
    sampled_dates = dates[::STEP]
    for idx, date in enumerate(sampled_dates):
        date_str = date.strftime("%Y-%m-%d")
        result[date_str] = compute_universe_slope_r2_index(components_by_ticker, date, tickers)
        if idx % 20 == 0:
...
def replay_ticker(ticker: str, tier: str, nifty_df: pd.DataFrame,
                   ...
                   slope_r2_index_by_date: DateIndexedRanks,
    ...
    for i in range(150, len(df) - FORWARD_BARS, STEP):
        ...
        slope_day = slope_r2_index_by_date.get_day(date_str)
        slope_r2_score = slope_day.get(ticker)  # None if missing — not zero-filled
        ...
        result = {
            ...
            "penalty_applied": penalty, "slope_r2_score": slope_r2_score,
...
def run_replay(tickers: list = None, fresh: bool = False) -> None:
    ...
    log.info("Precomputing rolling slope*R^2 per ticker (rolling OLS fit, once each)...")
    components_by_ticker = {
        t: compute_stock_slope_r2_components(df) for t, df in stock_data.items()
    }
    slope_r2_index_raw = _precompute_slope_r2_index_by_date(all_dates, components_by_ticker, list(stock_data.keys()))
    slope_r2_index_by_date = DateIndexedRanks(slope_r2_index_raw)

    log.info(f"Running slope*R^2 validation on {len(remaining)} stocks...")
    total_signals = 0

    for i, (ticker, name, sector, tier) in enumerate(remaining, 1):
        try:
            res = replay_ticker(ticker, tier, nifty_df, universe_ranks_by_date,
                                 slope_r2_index_by_date, regime_map)
...
def analyze_and_store() -> None:
    ...
    with_score = [o for o in all_outcomes if o.get("slope_r2_score") is not None]

    if len(with_score) < 30:
        log.warning(f"Only {len(with_score)} signals have a slope_r2_score — too few to analyze.")
        return

    report = {"generated_at": datetime.today().isoformat(),
              "total_signals": len(all_outcomes), "with_slope_r2_score": len(with_score)}
    rng = np.random.default_rng(RANDOM_SEED)

    pool_r = np.array([o["net_r"] for o in with_score], dtype=float)
    report["pool"] = _bucket_stats(with_score)

    values = np.array([o["slope_r2_score"] for o in with_score])
    p33, p67 = np.percentile(values, [33, 67])
    top    = [o for o in with_score if o["slope_r2_score"] >= p67]   # smoothest/strongest trend
    mid    = [o for o in with_score if p33 < o["slope_r2_score"] < p67]
    bottom = [o for o in with_score if o["slope_r2_score"] <= p33]   # weakest/choppiest trend
    ...
    report["top_tercile_slope_r2"] = top_stats
    report["mid_tercile_slope_r2"] = mid_stats
    report["bottom_tercile_slope_r2"] = bottom_stats
    ...
    conn = get_connection()
    conn.execute("""
        CREATE TABLE IF NOT EXISTS slope_r2_validate_results (
    ...
    """)
    conn.execute(
        "INSERT INTO slope_r2_validate_results (report_json, generated_at) VALUES (?,?)",
        (json.dumps(report), report["generated_at"])
    )
    conn.commit()
    conn.close()
    log.info("Analysis stored in slope_r2_validate_results.")


def print_stats() -> None:
    conn = get_connection()
    row = conn.execute(
        "SELECT report_json FROM slope_r2_validate_results ORDER BY id DESC LIMIT 1"
    ).fetchone()
    conn.close()
    if not row:
    ...
    print("\n" + "=" * 95)
    print("  FACTOR-LIBRARY BACKLOG (3) — Exponential Regression Slope x R^2 (Clenow, Stocks on the Move)")
    print("=" * 95)
    print(f"  Total gate-cleared signals: {report.get('total_signals', 0)} "
          f"({report.get('with_slope_r2_score', 0)} with a computable slope_r2_score)")
