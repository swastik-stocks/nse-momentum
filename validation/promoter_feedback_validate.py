"""
validation/promoter_feedback_validate.py — Factor-library item 4 validation

Tests PromoterFeedbackAgent (agents/promoter_feedback_agent.py) — Shu
(2009)'s MT-measure, adapted to promoter-holding delta since NSE's live
endpoint has no FII field (see that agent's docstring for the full
data-fit caveat) — against the same gate-cleared signal set items 1-3
used (pattern + RS + liquidity + risk + asymmetry + VCP,
price_history_deep, 2007-2026), reusing the real agent classes directly.

For every gate-cleared signal, this additionally computes MT from
shareholding_history (populated by load_shareholding_insider_history.py)
and buckets the SAME forward net-R outcomes by MT tercile — same
permutation-test methodology as tier1_factor_validate.py and
fifty_two_wk_high_validate.py. Deliberately does NOT presuppose Shu's
original direction (high MT = bad) — reports both terciles, lets the
permutation test speak for itself.

HONEST SCOPE LIMITATION: shareholding_history only has quarterly
granularity and only 492/500 tickers — signals for tickers with no
shareholding data, or without MIN_QUARTERS_FOR_SIGNAL quarters of history
before the signal date, are silently excluded (counted and reported, not
hidden).

Usage:
    python validation/promoter_feedback_validate.py
    python validation/promoter_feedback_validate.py RELIANCE
    python validation/promoter_feedback_validate.py --stats
    python validation/promoter_feedback_validate.py --fresh
"""
...
_log_file = _LOG_DIR / f"promoter_feedback_validate_{datetime.now().strftime('%Y-%m-%d_%H%M%S')}.log"
...
_PROGRESS_DDL = """
CREATE TABLE IF NOT EXISTS promoter_feedback_validate_progress (
...
def _clear_progress():
    conn = get_connection()
    conn.execute(_PROGRESS_DDL)
    conn.execute("DELETE FROM promoter_feedback_validate_progress")
    conn.commit()
    conn.close()
...
def _load_completed_tickers() -> set:
    conn = get_connection()
    rows = conn.execute("SELECT ticker FROM promoter_feedback_validate_progress").fetchall()
    conn.close()
...
def _save_ticker_progress(ticker: str, res: dict) -> None:
    conn = get_connection()
    conn.execute("""
        INSERT OR REPLACE INTO promoter_feedback_validate_progress
...
def _load_all_progress() -> list:
    conn = get_connection()
    rows = conn.execute(
        "SELECT ticker, signals, results_json FROM promoter_feedback_validate_progress"
    ).fetchall()
    conn.close()
    return [
...
class DateIndexedRanks:
    ...
    DateIndexedRanks for the full rationale (calendar mismatch between the
    per-date precompute and each ticker's own trading dates)."""
...
def _load_shareholding_by_ticker() -> dict:
    conn = get_connection()
    rows = conn.execute(
        "SELECT ticker, date, promoter_pct FROM shareholding_history ORDER BY ticker, date"
    ).fetchall()
    conn.close()
    by_ticker = {}
    for r in rows:
        by_ticker.setdefault(r["ticker"], []).append((r["date"], r["promoter_pct"]))
    return by_ticker


def _promoter_reading(series: list, date_str: str, quarters_back: int = QUARTERS_BACK):
    """series: [(date_str, promoter_pct), ...] sorted ascending for ONE ticker.
    Returns (promoter_now, promoter_trailing, quarters_available) or None."""
    if not series:
        return None
    dates = [d for d, _ in series]
    idx_now = bisect.bisect_right(dates, date_str) - 1
    if idx_now < 0:
        return None
    idx_trailing = max(0, idx_now - quarters_back)
    quarters_available = idx_now - idx_trailing
    if quarters_available < MIN_QUARTERS_FOR_SIGNAL:
        return None
    return series[idx_now][1], series[idx_trailing][1], quarters_available


def replay_ticker(ticker: str, tier: str, nifty_df: pd.DataFrame,
                   universe_ranks_by_date: DateIndexedRanks, regime_map: dict,
                   shareholding_series: list) -> dict:
    ...
    for i in range(150, len(df) - FORWARD_BARS, STEP):
        ...
        # ── NEW: item 4 diagnostic, computed on the SAME gate-cleared signal ──
        reading = _promoter_reading(shareholding_series, date_str)
        if reading is None:
            continue   # no usable promoter history for this ticker/date — excluded, not zero-filled
        promoter_now, promoter_trailing, quarters_avail = reading

        pf = PromoterFeedbackAgent(
            promoter_pct_now=promoter_now, promoter_pct_trailing=promoter_trailing,
            quarters_available=quarters_avail, rs_percentile=rsa.get_percentile()
        )
        if not pf.is_valid():
            continue
        ...
        outcome.update({
            ...
            "mt": pf.get_mt(),
            "delta": pf.get_delta(),
            "quarters_available": quarters_avail,
        })
...
def run_replay(tickers: list = None, fresh: bool = False) -> None:
    ...
    log.info("Loading shareholding history for all tickers...")
    shareholding_by_ticker = _load_shareholding_by_ticker()
    tickers_with_data = sum(1 for t, *_ in universe_items if t in shareholding_by_ticker)
    log.info(f"  {tickers_with_data}/{len(universe_items)} tickers have shareholding_history rows")
    ...
    log.info(f"Running promoter-feedback (item 4) validation on {len(remaining)} stocks...")
    total_signals = 0
    ...
    for i, (ticker, name, sector, tier) in enumerate(remaining, 1):
        try:
            res = replay_ticker(ticker, tier, nifty_df, universe_ranks_by_date, regime_map,
                                 shareholding_by_ticker.get(ticker, []))
        ...
        if i % 25 == 0:
            log.info(f"  {i}/{len(remaining)} | {total_signals:,} usable signals so far")

    log.info(f"Replay complete: {total_signals:,} usable signals (gate-cleared AND had promoter history)")
    analyze_and_store()
    print_stats()
...
def analyze_and_store() -> None:
    ...
    if not all_outcomes:
        log.warning("No usable signals found — nothing to analyze.")
        return
    ...
    # ── MT tercile split (Shu's original framing: high MT = feedback trading) ──
    top = df[df["mt"] >= df["mt"].quantile(2/3)]
    bot = df[df["mt"] <= df["mt"].quantile(1/3)]
    ...
    report["item4_mt_top_tercile_high"] = top_stats
    report["item4_mt_bottom_tercile_low"] = bot_stats

    top_sorted = top.sort_values("date")
    mid = len(top_sorted) // 2
    report["item4_top_tercile_first_half"] = _bucket_stats(top_sorted.iloc[:mid].to_dict("records"))
    report["item4_top_tercile_second_half"] = _bucket_stats(top_sorted.iloc[mid:].to_dict("records"))

    # ── 2x2: buying vs selling, in a momentum winner vs laggard ──
    buying_winner  = df[(df["delta"] > 0) & (df["mt"] > 0)]
    buying_laggard = df[(df["delta"] > 0) & (df["mt"] < 0)]
    selling_winner = df[(df["delta"] < 0) & (df["mt"] < 0)]
    selling_laggard= df[(df["delta"] < 0) & (df["mt"] > 0)]
    for name, sub in (("buying_winner", buying_winner), ("buying_laggard", buying_laggard),
                       ("selling_winner", selling_winner), ("selling_laggard", selling_laggard)):
        stats = _bucket_stats(sub.to_dict("records"))
        if stats["n"] >= 10:
            stats["p_value_vs_pool"] = round(
                _permutation_test(sub["net_r"].to_numpy(dtype=float), pool_r, N_PERM, rng), 4)
        report[f"item4_quadrant_{name}"] = stats

    conn = get_connection()
    conn.execute("""
        CREATE TABLE IF NOT EXISTS promoter_feedback_validate_results (
    ...
    """)
    conn.execute(
        "INSERT INTO promoter_feedback_validate_results (report_json, generated_at) VALUES (?,?)",
        (json.dumps(report), report["generated_at"])
    )
    conn.commit()
    conn.close()
    log.info("Analysis stored in promoter_feedback_validate_results.")


def print_stats() -> None:
    conn = get_connection()
    row = conn.execute(
        "SELECT report_json FROM promoter_feedback_validate_results ORDER BY id DESC LIMIT 1"
    ).fetchone()
    conn.close()
    if not row:
    ...
    print("\n" + "=" * 90)
    print("  FACTOR-LIBRARY ITEM 4 VALIDATION — promoter feedback-trading MT-measure (Shu 2009, adapted)")
    print("=" * 90)
